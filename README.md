...
```
* Задача 4:
На основании моделей из первой задачи сделать алгоритм, который по итогу будет показывать списки студентов с их оценки по каждому предмету. То есть грубо говоря, каждая строка  отчёта будет включать в себя название предмета, а дальше список имён студентов с их оценками. 
Например:
